 ...
 *
 *   bun run tools/cs2-export/dump-sticker-slots.ts
 *   bun run tools/cs2-export/dump-sticker-slots.ts --cs2 "D:/SteamLibrary/steamapps/common/Counter-Strike Global Offensive"
 *
 * Same overrides as the exporter: `--cs2` / `CS2_PATH` for the install, `--cli` /
 * `SOURCE2VIEWER_CLI` for the decompiler.
 */

import { mkdirSync, mkdtempSync, readdirSync, rmSync, statSync, writeFileSync } from 'node:fs'
import { tmpdir } from 'node:os'
import { join } from 'node:path'
import { cliPath, findCs2Pak, relSlash, requireCli } from './platform'
...
const main = async () => {
	...
	try {
		...
		const source = `// GENERATED by tools/cs2-export/dump-sticker-slots.ts — do not edit by hand.
//
