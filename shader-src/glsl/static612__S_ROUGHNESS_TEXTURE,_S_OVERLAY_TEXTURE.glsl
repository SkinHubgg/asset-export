...
// SPIR-V source (14880 bytes), GLSL reflection with SPIRV-Cross by KhronosGroup
// Static combos: S_ROUGHNESS_TEXTURE, S_OVERLAY_TEXTURE
...
layout(set = 1) uniform _1743 _Globals_;
...
layout(set = 1, binding = 36) uniform texture2D g_tPaintRoughness;
...
void main()
{
    ...
    if (_Globals_.bRoughnessMode != 0)
    {
        ...
        _19391.x = _19391.x;
        vec4 _19068 = texture(sampler2D(g_tPaintRoughness, g_sTrilinearWrap), input_1.xy);
        float _21710;
        SPIRV_CROSS_BRANCH
