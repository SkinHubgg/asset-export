...
// SPIR-V source (10040 bytes), GLSL reflection with SPIRV-Cross by KhronosGroup
// Static combos: S_PAINT_STYLE=8, S_PEARLESCENCE_MASK
...
layout(set = 1) uniform _2356 _Globals_;
...
void main()
{
    ...
    if (_Globals_.bRoughnessMode != 0)
    {
        ...
        {
            vec4 _18602 = _13436;
            _18602.z = _7959 * texture(sampler2D(g_tPearlescenceMask, g_sTrilinearWrap), input_1.zw).x;
            _21709 = _18602;
        }
        else
        {
            float _24237 = _7959 * texture(sampler2D(g_tPearlescenceMask, g_sTrilinearWrap), input_1.xy).x;
            vec4 _9184 = _13436;
