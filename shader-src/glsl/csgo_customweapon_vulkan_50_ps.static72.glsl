...
// SPIR-V source (7528 bytes), GLSL reflection with SPIRV-Cross by KhronosGroup
// Static combos: S_PEARLESCENCE_MASK
...
layout(set = 1) uniform _2733 _Globals_;
...
layout(set = 1, binding = 36) uniform texture2D g_tPearlescenceMask;
...
void main()
{
    ...
    vec4 _20360 = texture(sampler2D(g_tWear, g_sTrilinearWrap), input_2.xy);
    ...
    float _22167 = smoothstep(0.579999983310699462890625 - _4693, 0.680000007152557373046875 + _4693, ((_18992.w + (_20360.x * _4306)) * ((_Globals_.g_flWearAmount * 6.0) + 1.0)) * _10779);
    ...
    if (_Globals_.bRoughnessMode != 0)
    {
        ...
        {
            _20271 = mix(mix(mix(_Globals_.g_vPaintRoughness.x, _Globals_.g_vPaintRoughness.y, _7656), _Globals_.g_vPaintRoughness.z, _11437), _Globals_.g_vPaintRoughness.w, _12425);
        }
        else
        {
            _20271 = _Globals_.g_flPaintRoughness;
        }
        vec4 _13720 = mix(vec4(min(1.0, _20271 + ((((1.0 - _16529.w) * _Globals_.g_flWearAmount) * _Globals_.g_flWearAmount) * 0.5)), _7177, 0.0, 1.0), _19391, vec4(_22167));
        vec4 _20361 = texture(sampler2D(g_tPearlescenceMask, g_sTrilinearWrap), input_1.xy);
        float _13666 = (1.0 - _22167) * _20361.x;
        vec4 _8739 = _13720;
        _8739.z = _13666;
        vec3 _18043 = _8739.xyz * vec3(0.077399380505084991455078125);
        vec3 _7676 = pow((_8739.xyz * vec3(0.947867333889007568359375)) + vec3(0.052132703363895416259765625), vec3(2.400000095367431640625));
        float _21354;
        if (_13720.x <= 0.040449999272823333740234375)
        {
            _21354 = _18043.x;
        }
        else
        ...
        float _23035;
        if (_13720.y <= 0.040449999272823333740234375)
        {
            _23035 = _18043.y;
        }
        else
        ...
        float _19513;
        if (_13666 <= 0.040449999272823333740234375)
        {
            _19513 = _18043.z;
        }
        else
        ...
        vec4 _17156 = vec4(_21354, _23035, _13666, min(1.0, _Globals_.g_flPearlescentScale));
        _17156.z = _19513;
        _7078 = _17156;
    }
    else
    ...
    if (_Globals_.bRoughnessMode == 0)
    {
        vec3 _13063 = mix(mix(mix(_Globals_.g_vColor0, _Globals_.g_vColor1, vec3(_7656)), _Globals_.g_vColor2, vec3(_11437)), _Globals_.g_vColor3, vec3(_12425));
        vec3 _21103 = _13063.xyz * _16529.xyz;
