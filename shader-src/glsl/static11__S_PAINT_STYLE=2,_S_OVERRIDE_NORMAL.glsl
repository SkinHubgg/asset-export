...
// SPIR-V source (18456 bytes), GLSL reflection with SPIRV-Cross by KhronosGroup
// Static combos: S_PAINT_STYLE=2, S_OVERRIDE_NORMAL
...
layout(set = 1) uniform _1103 _Globals_;
...
void main()
{
    ...
    for (;;)
    {
        ...
        _19130 = _13155 + (texture(sampler2D(g_tPosition, g_sTrilinearClamp), (input_1.xy + (_2920[_16208] * 0.20000000298023223876953125)).xy) * 0.0588235296308994293212890625);
        _18752 = _16208 + 1;
        _13155 = _19130;
        _16208 = _18752;
        continue;
    ...
    float _10779 = mix(mix(mix(_Globals_.g_vPaintDurability.x, _Globals_.g_vPaintDurability.y, _11237.x), _Globals_.g_vPaintDurability.z, _11237.y), _Globals_.g_vPaintDurability.w, _11237.z);
    float _5486 = ((_19372.w + (_19068.x * _4306)) * ((_Globals_.g_flWearAmount * 6.0) + 1.0)) * _10779;
    ...
    vec3 _21103 = _11237.xyz * vec3(smoothstep(0.579999983310699462890625 + _4693, _25193 - _4693, _5486), smoothstep(_25193 + _4693, _17192 - _4693, _5486), smoothstep(_17192 + _4693, (0.519999980926513671875 - (_Globals_.g_flWearAmount * 0.17999999225139617919921875)) - _4693, _5486)).xyz;
    float _12931 = _21103.x;
    float _13767 = _21103.y;
    float _14497 = _21103.z;
    float _21647 = smoothstep(0.579999983310699462890625 - _4693, 0.680000007152557373046875 + _4693, _5486);
    ...
    if (_Globals_.bRoughnessMode != 0)
    {
        ...
        {
            _20271 = mix(mix(mix(_Globals_.g_vPaintRoughness.x, _Globals_.g_vPaintRoughness.y, _12931), _Globals_.g_vPaintRoughness.z, _13767), _Globals_.g_vPaintRoughness.w, _14497);
        }
        else
        {
            _20271 = _Globals_.g_flPaintRoughness;
        }
        vec4 _14119 = mix(vec4(min(1.0, _20271 + ((((1.0 - _16529.w) * _Globals_.g_flWearAmount) * _Globals_.g_flWearAmount) * 0.5)), _7177, 0.0, 1.0), _19391, vec4(_21647));
        float _10938 = 1.0 - _21647;
        vec4 _13018 = _14119;
        _13018.z = _10938;
