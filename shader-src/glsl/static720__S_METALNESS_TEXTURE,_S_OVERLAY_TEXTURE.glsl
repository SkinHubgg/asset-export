...
// SPIR-V source (15004 bytes), GLSL reflection with SPIRV-Cross by KhronosGroup
// Static combos: S_METALNESS_TEXTURE, S_OVERLAY_TEXTURE
...
layout(set = 1) uniform _1647 _Globals_;
...
layout(set = 1, binding = 36) uniform texture2D g_tPaintMetalness;
...
void main()
{
    ...
    {
        _21710 = texture(sampler2D(g_tPaintMetalness, g_sTrilinearWrap), input_1.zw).x;
    }
    else
    {
        _21710 = texture(sampler2D(g_tPaintMetalness, g_sTrilinearWrap), input_1.xy).x;
    }
    float _10247;
    vec4 _11711;
    if (_Globals_.bRoughnessMode != 0)
    {
        ...
        {
            _21711 = mix(mix(mix(_Globals_.g_vPaintRoughness.x, _Globals_.g_vPaintRoughness.y, _7656), _Globals_.g_vPaintRoughness.z, _11437), _Globals_.g_vPaintRoughness.w, _12425);
        }
        else
        {
        ...
        float _13705 = mix(_21710, _Globals_.g_fOverlayMetalness, _10825);
        vec4 _13796 = mix(vec4(min(1.0, mix(_21711, _Globals_.g_fOverlayRoughness, _10825) + ((((1.0 - _16529.w) * _Globals_.g_flWearAmount) * _Globals_.g_flWearAmount) * 0.5)), _13705, 0.0, 1.0), _19391, vec4(_21709));
