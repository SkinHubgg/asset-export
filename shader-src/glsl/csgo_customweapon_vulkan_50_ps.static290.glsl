...
// SPIR-V source (21908 bytes), GLSL reflection with SPIRV-Cross by KhronosGroup
// Static combos: S_PAINT_STYLE=2, S_SEPARATE_CHANNEL_INPUTS
...
void main()
{
    ...
    for (;;)
    {
        ...
        _19130 = _13155 + (texture(sampler2D(g_tPosition, g_sTrilinearClamp), (input_1.xy + (_2953[_16208] * 0.20000000298023223876953125)).xy) * 0.0588235296308994293212890625);
        _18752 = _16208 + 1;
        _13155 = _19130;
        _16208 = _18752;
        continue;
    ...
    float _10779 = mix(mix(mix(_Globals_.g_vPaintDurability.x, _Globals_.g_vPaintDurability.y, _11692.x), _Globals_.g_vPaintDurability.z, _11692.y), _Globals_.g_vPaintDurability.w, _11692.z);
    float _5486 = ((_18087.w + (_19068.x * mix(smoothstep(0.0, 0.680000007152557373046875, pow(_4306, 1.10000002384185791015625)), smoothstep(0.0, 0.5099999904632568359375, pow(_4306, 0.800000011920928955078125)), pow(_Globals_.g_flWearAmount, 0.300000011920928955078125)))) * ((_Globals_.g_flWearAmount * 6.0) + 1.0)) * _10779;
    ...
    if (_Globals_.bRoughnessMode != 0)
    {
        ...
        vec4 _14119 = mix(vec4(min(1.0, _20271 + ((((1.0 - _16529.w) * _Globals_.g_flWearAmount) * _Globals_.g_flWearAmount) * 0.5)), _7177, 0.0, 1.0), _19391, vec4(_20939));
