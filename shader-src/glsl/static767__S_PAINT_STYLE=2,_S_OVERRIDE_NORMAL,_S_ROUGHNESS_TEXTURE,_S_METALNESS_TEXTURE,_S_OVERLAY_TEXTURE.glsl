...
// SPIR-V source (27212 bytes), GLSL reflection with SPIRV-Cross by KhronosGroup
// Static combos: S_PAINT_STYLE=2, S_OVERRIDE_NORMAL, S_ROUGHNESS_TEXTURE, S_METALNESS_TEXTURE, S_OVERLAY_TEXTURE
...
layout(set = 1) uniform _1030 _Globals_;
...
void main()
{
    ...
    for (;;)
    {
        ...
        _19130 = _13155 + (texture(sampler2D(g_tPosition, g_sTrilinearClamp), (input_1.xy + (_151[_16208] * 0.20000000298023223876953125)).xy) * 0.0588235296308994293212890625);
        _18752 = _16208 + 1;
        _13155 = _19130;
        _16208 = _18752;
        continue;
    ...
    vec2 _14674 = vec2(dot(_18941.yz, _Globals_.g_vPatternTexCoordXform0.xy) + _Globals_.g_vPatternTexCoordXform0.w, dot(_18941.yz, _Globals_.g_vPatternTexCoordXform1.xy) + _Globals_.g_vPatternTexCoordXform1.w);
    vec4 _19680 = texture(sampler2D(g_tPattern, AddressU_dynamic_AddressV_dynamic), _14674);
    vec2 _20282 = vec2(dot(_18941.xz, _Globals_.g_vPatternTexCoordXform0.xy) + _Globals_.g_vPatternTexCoordXform0.w, dot(_18941.xz, _Globals_.g_vPatternTexCoordXform1.xy) + _Globals_.g_vPatternTexCoordXform1.w);
    vec4 _19681 = texture(sampler2D(g_tPattern, AddressU_dynamic_AddressV_dynamic), _20282);
    vec2 _20283 = vec2(dot(_18941.yx, _Globals_.g_vPatternTexCoordXform0.xy) + _Globals_.g_vPatternTexCoordXform0.w, dot(_18941.yx, _Globals_.g_vPatternTexCoordXform1.xy) + _Globals_.g_vPatternTexCoordXform1.w);
    vec4 _19718 = texture(sampler2D(g_tPattern, AddressU_dynamic_AddressV_dynamic), _20283);
    ...
    if (_Globals_.bRoughnessMode != 0)
    {
        ...
        {
            vec3 _10153 = normalize((texture(sampler2D(g_tSurface, g_sTrilinearClamp), input_1.xy).xyz * 2.0) - vec3(1.0));
            _21712 = mix(mix(texture(sampler2D(g_tPaintRoughness, g_sTrilinearWrap), _14674).xyzw, texture(sampler2D(g_tPaintRoughness, g_sTrilinearWrap), _20282).xyzw, vec4(_Globals_.g_vSprayBiasBlend.y * pow(abs(_10153.y), 7.0))), texture(sampler2D(g_tPaintRoughness, g_sTrilinearWrap), _20283).xyzw, vec4(_Globals_.g_vSprayBiasBlend.z * pow(abs(_10153.z), 7.0))).x;
        }
        else
        {
        ...
        {
            _21714 = mix(mix(mix(_Globals_.g_vPaintRoughness.x, _Globals_.g_vPaintRoughness.y, _12931), _Globals_.g_vPaintRoughness.z, _13767), _Globals_.g_vPaintRoughness.w, _14497);
        }
        else
        {
            _21714 = _21712;
        ...
        vec4 _13796 = mix(vec4(min(1.0, mix(_21714, _Globals_.g_fOverlayRoughness, _10825) + ((((1.0 - _16529.w) * _Globals_.g_flWearAmount) * _Globals_.g_flWearAmount) * 0.5)), _13705, 0.0, 1.0), _19391, vec4(_21710));
    ...
    if (_Globals_.bRoughnessMode == 0)
    {
        ...
        switch (_Globals_.nOverlayBlendMode)
        {
            case 0:
            {
                _21715 = mix(_18755.xyz, _19287.xyz, vec3(_19287.w));
                break;
            }
            case 1:
            {
            ...
            case 3:
            {
                ...
                _21715 = mix(_13962, mix(vec3(_Globals_.g_vAlbedoLevels.x), mix(_19113, min(_Globals_.g_vAlbedoLevels.zzz, _19113 + (vec3(_7243) * 2.0)), vec3(1.0 / _Globals_.g_fColorBoostFactor)).xyz, vec3(pow(smoothstep(0.0003000000142492353916168212890625, _Globals_.g_vAlbedoLevels.x, _7243), 0.5))).xyz, vec3(_19287.w));
                break;
            }
            case 4:
            {
        ...
        vec3 _19616 = saturate(_21715 * mix(_Globals_.g_flColorBrightness, 1.0, smoothstep(0.0, 0.00999999977648258209228515625, _21647) * (1.0 - (smoothstep(0.5, 0.60000002384185791015625, _22743.w) * smoothstep(1.0, 0.89999997615814208984375, _22743.w)))));
