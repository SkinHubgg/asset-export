...
// cbuffer ID 5618 (name: _Globals_), member index 17 (name: g_vMetallicPaintAlbedoLevels) cannot be expressed with either HLSL packing layout or packoffset.
//
// Re-attempting reflection with the GLSL backend.

// Source 2 Viewer 19.2.0.0 - https://valveresourceformat.github.io
// SPIR-V source (13516 bytes), GLSL reflection with SPIRV-Cross by KhronosGroup
// Static combos: S_PAINT_STYLE=6, S_OVERLAY_TEXTURE
...
layout(set = 1) uniform _2522 _Globals_;
...
void main()
{
    ...
    if (_Globals_.bRoughnessMode != 0)
    {
        ...
        float _13705 = mix(_Globals_.g_flPaintMetalness, _Globals_.g_fOverlayMetalness, _10825);
        vec4 _13796 = mix(vec4(min(1.0, mix(_Globals_.g_flPaintRoughness, _Globals_.g_fOverlayRoughness, _10825) + ((((1.0 - _16529.w) * _Globals_.g_flWearAmount) * _Globals_.g_flWearAmount) * 0.5)), _13705, 0.0, 1.0), _19391, vec4(_21710));
    ...
    else
    {
        ...
        _10247 = _Globals_.g_flPaintMetalness;
    }
    vec4 _22401;
    if (_Globals_.bRoughnessMode == 0)
    {
